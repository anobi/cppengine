...
void Rendering::GL::GLPointLight::UpdateShaderAttributes(std::shared_ptr<Shader> shader) {
...
void Rendering::GL::GLDirectionalLight::UpdateShaderAttributes(std::shared_ptr<Shader> shader)
{
	int loc = this->uniform_location;
	glUniform3fv(shader->uniforms[loc + 0], 1, &this->_light->GetTransform().GetPosition()[0]);
	glUniform3fv(shader->uniforms[loc + 1], 1, &this->_light->GetColor()[0]);
